...
export default async function handler(req, res) {
  ...
  try {
    const { image, style, emotions: requestedEmotions } = req.body;
    ...
    if (!requestedEmotions || !Array.isArray(requestedEmotions) || requestedEmotions.length === 0) {
    ...
    const results = await Promise.all(
      requestedEmotions.map(emotion => generateStickerForEmotion(image, style, emotion, geminiApiKey))
    );
